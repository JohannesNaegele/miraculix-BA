## This script compares different tilesizes, number of streams and gemm shapes for MMAGPU. 
        ...
        for (i in 1:average_matrix) {
          ...
          tile_time[shape,tile,stream] <- tile_time[shape,tile,stream] + system.time({
            G <- miraculix::relationshipMatrix(Z,
              shape = shapes[shape], n_streams = streams[stream], tilesize = tiles[tile]
            )
          })[3]
        }
...
rm(Z)
saveRDS(tile_time, "tile_time_old.rds")
