...
relationshipMatrix <- function(SNPxIndiv, warp = FALSE, shape = 0, naive = TRUE, tilesize=2048, n_streams=2, ...) {
  ...
  opt <- RFopt$basic
  wmma <- if (is.logical(warp)) warp else FALSE
  nv <- if (is.logical(naive)) naive else TRUE
  sh <- if (is.numeric(shape) && (shape%%1==0) && shape >= 0) shape else 0
  ...
  else A <- .Call(C_crossprod, SNPxIndiv, wmma, nv, sh, tilesize, n_streams)
