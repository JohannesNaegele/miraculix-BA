...
Uint GetBytesPerBlock(snpcoding method) {
  ...
  case Packed256: case MMAGPU : case MMA1Bit :return BytesPerBlockPacked256();
...
Uint GetCodesPerBlock(snpcoding method) {
  ...
  case Packed256: case MMAGPU : case MMA1Bit : return CodesPerBlockPacked256();
...
Uint GetBitsPerCode(snpcoding method) {
  ...
  case Haplo : case Multiply : case Multiply256 : case MMAGPU : case MMA1Bit :
    return BitsPerCode2();
...
Uint GetUPI(Uint snps, snpcoding method) {
  ...
  case MMA1Bit :
    return UnitsPerIndiv256_1(snps);
...
Ulong sumGeno(Uint * SNPxIndiv, Uint snps, Uint individuals, snpcoding method) {
  ...
  case Multiply : case Multiply256 : case MMAGPU : case MMA1Bit :
...
Uint *AlignBase(SEXP CM, Uint VARIABLE_IS_NOT_USED nr, Uint alignment, bool test) {
  ...
  Uint bytes; 
  if (METHOD == MMA1Bit) {
    bytes = 2*MemInUnits(info) * BytesPerUnit;
  } else {
    bytes = MemInUnits(info) * BytesPerUnit;
  }
#ifdef DO_PARALLEL
...
Uint* DoAlign(SEXP CM, Uint nr, snpcoding method, bool test) {
  ...
  case MMA1Bit : return (Uint*) INTEGER(CM);
...
void cp_dummy(Uint *CGM, Uint snps, Uint individuals, double *ans) {
}

void crossprod(Uint * SNPxIndiv, Uint snps, Uint individuals,
		 snpcoding method,  bool centred, bool normalized,
		 Ulong SumGeno, double *A, bool warp, bool naive, unsigned int shape, size_t tilesize, size_t n_streams) {
  ...
  case MMAGPU : {
    cp = cp_dummy;
    crossprod_mmagpu(SNPxIndiv, snps, individuals, A, shape, tilesize, n_streams);
    break;
  }
  case MMA1Bit : {
    cp = cp_dummy;
    crossprod_mma1Bit(SNPxIndiv, snps, individuals, A, warp, naive, shape, tilesize, n_streams);
    break;
  }
  ...
  assert(A[0] >= 0.0);
  if (centred || normalized) {
    ERR("Centering and normalization not yet implemented.");
...
double *crossprod(Uint * SNPxIndiv, Uint snps, Uint individuals,
		    snpcoding method,  bool centred, bool normalized,
		    Ulong SumGeno, bool warp, bool naive, unsigned int shape, size_t tilesize, size_t n_streams) {
  ...
  crossprod(SNPxIndiv, snps, individuals, method, centred, normalized,
	      SumGeno, A, warp, naive, shape, tilesize, n_streams);
...
double *crossprod(Uint * SNPxIndiv, Uint snps, Uint individuals,
		    snpcoding method, bool centred, bool normalized,
		    Ulong SumGeno) {
  return crossprod(SNPxIndiv, snps, individuals,
		    method,  centred, normalized,
		    SumGeno, false, true, 0, 2048, 1);
}
 

SEXP crossprod(SEXP SNPxIndiv, SEXP warp, SEXP naive, SEXP shape, SEXP tilesize, SEXP n_streams) {
  size_t ts = asInteger(tilesize);
  size_t nstr = asInteger(n_streams);
  bool wmma = asLogical(warp);
  bool nv = asLogical(naive);
  unsigned int sh = asInteger(shape);
   Uint
  ...
  crossprod(code, snps, individuals, method,
	      ...
	      SumGeno(info), REAL(Ans), wmma, nv, sh, ts, nstr);
...
SEXP file_get(SEXP file) {
  ...
  switch (method) {
  case Shuffle256 : case Shuffle :  case TwoBit : case Packed : case MMAGPU : case MMA1Bit : 
    if (useMMA1Bit(method)) 
      return file_intern(file, matrix_start_mma1Bit, coding2,
...
SEXP createSNPmatrix(Uint snps, Uint individuals, snpcoding method) {  
  ...
  case MMA1Bit : start = matrix_start_mma1Bit; break;
...
SEXP fillSNPmatrix(SEXP Z, SEXP Idx, SEXP Vector) {  
  ...
  if (methZ != MMA1Bit) {
    ...
    snpcoding methVector = check_method((snpcoding) infoVector[METHOD]);
    ...
    for (Ulong i=0; i<len; i++) {
      Uint idx = Idxint[i] - 1;
      if (idx >= individuals) ERR("Idx out of bound");
      // printf("len: %u idx: %u, individuals: %u\n", len, idx, individuals);
      // input is raw Vector
      coding1(((Uint*) INTEGER(Vector)) + i*snpsZ, idx, idx+1, 0, snpsZ, snpsZ, Z, NULL);
    }
  }
  return R_NilValue;
}
...
Ulong calculateAlignedMem(Ulong memInUnits, snpcoding method,
			  Uint bytesPerBlock) {
  if (is256(method) && method != MMA1Bit)
    return (2L + (memInUnits - 1L) / UnitsPerBlock256) *  UnitsPerBlock256 - 1L;
  if (is256(method) && method == MMA1Bit)
    return ((memInUnits - 1L) / UnitsPerBlock256 + 1) *  UnitsPerBlock256;
...
SEXP CreateEmptyCodeVector(Uint snps, Uint individuals, snpcoding method) {
  ...
  // ERR2("method mismatch (%i; %i). Pls contact author",
	// method, Haplo);
  Ulong
    unitsPerIndiv = GetUPI(snps, method),
    memInUnits = (Ulong) individuals * unitsPerIndiv;
  ...
  if (method == MMA1Bit) {
    alignedMem *= 2; // 1-bit needs twice the amount bc of decomposition
  }
  ...
  StoreAlignedInUnits(alignedMem); // is not used further
